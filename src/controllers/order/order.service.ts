...
import {
	Cart, Order, Product, ProductDocument, ProductVariables, UserDocument
} from '../../models'
...
export const checkMakeOrderValues = (user: UserDocument, context: any) => {
	...
	return Cart.findOne({ userId: user._id.toString() }).then((cartObj) => {
		if (!cartObj || !(cartObj.cart.length > 0)) {
			...
			return createCart(cartObj.cart).then((cart) => ({ cart, selectedAddress, card: context.card }))
		}
	})
}
...
export const getAllOrders = () => Order.find().sort({ _id: -1 })
